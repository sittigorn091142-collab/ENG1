            ...
            <div className="flex items-center gap-6 mt-4 md:mt-0">
              <span
                className={`px-4 py-1.5 rounded-full text-xs font-black uppercase border-2 ${
                  b.status &&
                  (b.status.includes("แล้ว") || b.status.includes("Completed"))
            ...
            >
              ...
              <div className="flex-1">
                <div className="h-2 bg-slate-100 rounded-full overflow-hidden mb-2">
                  <div
                    className={`h-full ${
                      task.status === "Done" ? "bg-green-500" : "bg-indigo-500"
                    }`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-8 rounded-[2rem] border shadow-sm">
          ...
          <div className="space-y-4">
            {Object.entries(
              authorizedUsers.reduce((acc, u) => ({ ...acc, [u.name]: 0 }), {})
            ).map(([name, _]) => {
              const count = tasks.filter(
                (t) =>
                  (t.status === "In Progress" || t.status === "To Do") &&
                  (t.assignee === name || t.assignee?.includes(name))
              ).length;
              return (
      ...
      <div className="bg-white p-8 rounded-[2.5rem] w-full max-w-lg shadow-2xl animate-in zoom-in-95 h-[80vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pr-2">
          <div className="grid grid-cols-2 gap-4">
            <InfoBlock label="ผู้รับผิดชอบ" val={task.assignee} />
            <InfoBlock label="กำหนดส่ง" val={safeDate(task.dueDate)} />
            <InfoBlock
              label="ความสำคัญ"
              val={<PriorityBadge p={task.priority} />}
            />
      ...
      previewData.forEach((cols) => {
        if (cols.length >= 4 && cols[0].toUpperCase() !== "STORE CODE") {
          ...
          batch.set(docRef, {
            ...
            status:
              targetProject === PROJECT_TYPES.REPAIR
                ? "ยังไม่ดำเนินการ"
                : "ยังไม่เสนอราคา",
            remarks: "",
            fileLinks: [],
    ...
    <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl w-full max-w-2xl animate-in zoom-in-95">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-black text-slate-800 flex items-center gap-2">
            <Database size={24} className="text-blue-500" /> Import Data
          </h3>
          <button onClick={onClose}>
        ...
        <div className="mb-6 flex gap-2 p-1 bg-slate-100 rounded-xl w-fit">
          <button
            onClick={() => setTargetProject(PROJECT_TYPES.REPAIR)}
          ...
          <button
            onClick={() => setTargetProject(PROJECT_TYPES.REPLACE)}
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${
              targetProject === PROJECT_TYPES.REPLACE
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-2 p-4 bg-green-600 text-white rounded-xl font-bold shadow-lg hover:bg-green-700"
              >
                ยืนยันนำเข้า
              </button>
            </div>
