...
const callGemini = async (prompt) => {
  try {
    ...
    return data.candidates?.[0]?.content?.parts?.[0]?.text || "ไม่สามารถสร้างข้อความได้";
  } catch (error) {
...
const AUTHORIZED_USERS = [
  { id: "7515698", name: "นายวันชัย ศรีเจียม", position: "GM วิศวกรรมวิเคราะห์ Call" },
  ...
  { id: "7512643", name: "นายชิษณุพงษ์ วิริยะพันธ์", position: "Supportวิเคราะห์ Call" }
];
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        unsubTasks = onSnapshot(query(collection(db, "internal_tasks"), orderBy("createdAt", "desc")), (snap) => {
    ...
    setConfirmModal({
      ...
      onConfirm: async (adminCode, selectedProject) => {
        ...
        try {
          ...
          const toDelete = branchData.filter(b => b.projectType === selectedProject);
          if (toDelete.length === 0) return alert("ไม่พบข้อมูลที่จะลบในโปรเจคนี้");

          setMessage({ type: "info", text: `กำลังลบข้อมูล ${toDelete.length} รายการ...` });
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center">
        ...
        <p className="text-slate-500 font-bold animate-pulse">กำลังเชื่อมต่อระบบ...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-center p-6 font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-6xl px-4">
          ...
          <MenuCard onClick={() => setRole("tracker")} icon={<MapPin size={56} />} title="Projects ลดCallแจ้งซ่อมวิศวกรรม" desc="ระบบ Tracking รายสาขา" color="bg-orange-500 text-white shadow-orange-200 hover:bg-orange-600" />
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-12">
      <Header role={role} isAdmin={isAdminLoggedIn} user={currentUser} onLogout={() => { setIsAdminLoggedIn(false); setCurrentUser(null); setRole(null); }} onBack={() => { setRole(null); setIsAdminLoggedIn(false); }} onImport={() => setShowImportModal(true)} />
...
const TrackerView = ({ branches, filteredBranches, projectType, setProjectType, search, setSearch, area, setArea, team, setTeam, onEdit, onExport, onDeleteAll }) => {
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div className="flex gap-2 p-1 bg-white rounded-xl shadow-sm border w-fit">
          <button onClick={() => setProjectType(PROJECT_TYPES.REPAIR)} className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${projectType === PROJECT_TYPES.REPAIR ? "bg-orange-500 text-white" : "text-slate-400"}`}>🛠️ เสียดสี (Elite v.3)</button>
          <button onClick={() => setProjectType(PROJECT_TYPES.REPLACE)} className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${projectType === PROJECT_TYPES.REPLACE ? "bg-orange-500 text-white" : "text-slate-400"}`}>📄 OPENTYPE 10Y+ (ประเมินอุปกรณ์)</button>
        </div>
        <div className="flex gap-2">
          <button type="button" onClick={onDeleteAll} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-xl font-bold flex items-center gap-2 shadow-lg transition-all text-sm"><Trash2 size={16}/> ล้างข้อมูล (Admin)</button>
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        )) : <div className="text-center p-12 text-slate-400 font-bold bg-white rounded-[2rem] border border-dashed">ไม่พบข้อมูลสาขา</div>}
      </div>
    </div>
  );
};
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y text-sm">
            ...
            )) : <tr><td colSpan="4" className="p-12 text-center text-slate-400 font-bold">ไม่พบข้อมูลรายงาน</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
            ...
            <div className="space-y-3">
              {tasks.filter(t => t.status === status).map(task => (
                <div key={task.id} onClick={() => onEdit(task)} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-all cursor-pointer">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-slate-50"><div className="flex items-center gap-1 text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg truncate max-w-[150px]"><User size={12} /> {task.assignees ? (Array.isArray(task.assignees) ? task.assignees.join(", ") : task.assignees) : (task.assignee ? task.assignee.split(" ")[0] : "?")}</div></div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-8 rounded-[2rem] border shadow-sm">
          <div className="flex justify-between items-center mb-6">
             <h4 className="font-black text-slate-800 text-lg flex items-center gap-2">Team Workload</h4>
             <div className="flex gap-2">
                <select className="p-2 border rounded-lg text-xs" value={filterYear} onChange={e => setFilterYear(parseInt(e.target.value))}>
                   {[2023, 2024, 2025, 2026].map(y => <option key={y} value={y}>{y}</option>)}
                </select>
                <select className="p-2 border rounded-lg text-xs" value={filterMonth} onChange={e => setFilterMonth(parseInt(e.target.value))}>
                   {["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"].map((m, i) => <option key={i} value={i}>{m}</option>)}
                </select>
             </div>
          </div>
          <div className="space-y-4">
            {Object.entries(authorizedUsers.reduce((acc, u) => ({ ...acc, [u.name]: { total: 0, done: 0 } }), {})).map(([name, counts]) => {
              const userTasks = tasks.filter(t => {
                 const tDate = t.startDate ? new Date(t.startDate) : new Date(t.createdAt);
                 const inPeriod = tDate.getFullYear() === filterYear && tDate.getMonth() === filterMonth;
                 const isAssignee = t.assignees ? (Array.isArray(t.assignees) ? t.assignees.includes(name) : t.assignees === name) : (t.assignee && t.assignee.includes(name));
              ...
              return (
                <div key={name} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 flex gap-1 h-3 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-green-500" style={{ width: total > 0 ? `${(done / total) * 100}%` : '0%' }}></div>
                    <div className="h-full bg-indigo-500" style={{ width: total > 0 ? `${((total - done) / total) * 100}%` : '0%' }}></div>
                  </div>
                  <div className="text-xs font-black text-slate-800 w-16 text-right">{done} / {total}</div>
                </div>
              );
            })}
          </div>
          <div className="flex gap-4 mt-6 justify-center text-[10px] font-bold text-slate-400">
             <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-green-500"></div> เสร็จสิ้น</div>
             <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-indigo-500"></div> คงเหลือ</div>
          </div>
        </div>
      </div>
    )}
...
const ConfirmationModal = ({ title, msg, onConfirm, onClose, showInput, extraInputs }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl w-full max-w-sm text-center animate-in zoom-in-95">
        ...
        {extraInputs && (
           <div className="mb-4">
              <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">เลือกโปรเจคที่จะลบ</label>
              <select className="w-full p-2 border rounded-xl font-bold text-sm mb-3" value={projType} onChange={e => setProjType(e.target.value)}>
...
const TaskDetailModal = ({ task, onClose, currentUser, setConfirmModal }) => {
  ...
  const toggleAssignee = (name) => {
     ...
     if (current.includes(name)) {
        setFormData({ ...formData, assignees: current.filter(n => n !== name) });
     } else {
    ...
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-[2.5rem] w-full max-w-lg shadow-2xl animate-in zoom-in-95 h-[85vh] flex flex-col">
        <div className="flex justify-between items-start mb-4">
          <input className="text-xl font-black text-slate-800 w-full bg-transparent border-b-2 border-transparent focus:border-indigo-500 outline-none" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pr-2">
          ...
          <div className="grid grid-cols-2 gap-4">
             <div>
                ...
                <select className="w-full p-2 border rounded-lg font-bold text-sm" value={formData.priority} onChange={e => setFormData({...formData, priority: e.target.value})}><option value="High">High</option><option value="Medium">Medium</option><option value="Low">Low</option></select>
             </div>
             <div>
                <label className="text-[10px] font-black text-slate-400 uppercase">Status</label>
                <select className="w-full p-2 border rounded-lg font-bold text-sm" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})}><option value="To Do">To Do</option><option value="In Progress">In Progress</option><option value="Review/QC">Review/QC</option><option value="Done">Done</option></select>
             </div>
             <div><label className="text-[10px] font-black text-slate-400 uppercase">Start</label><input type="date" className="w-full p-2 border rounded-lg text-sm" value={formData.startDate} onChange={e => setFormData({...formData, startDate: e.target.value})} /></div>
             <div><label className="text-[10px] font-black text-slate-400 uppercase">Due</label><input type="date" className="w-full p-2 border rounded-lg text-sm" value={formData.dueDate} onChange={e => setFormData({...formData, dueDate: e.target.value})} /></div>
          </div>

          <div>
             <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block">ผู้รับผิดชอบ (เลือกได้หลายคน)</label>
             <div className="p-3 border rounded-xl max-h-32 overflow-y-auto grid grid-cols-2 gap-2">
                {AUTHORIZED_USERS.map(u => (
                   <label key={u.id} className={`flex items-center gap-2 text-xs font-bold p-2 rounded-lg cursor-pointer ${formData.assignees.includes(u.name) ? "bg-indigo-50 text-indigo-700" : "bg-slate-50 text-slate-500"}`}>
                      <input type="checkbox" checked={formData.assignees.includes(u.name)} onChange={() => toggleAssignee(u.name)} className="accent-indigo-600" />
                      {u.name}
                   </label>
                ))}
             </div>
          ...
          <div className="relative">
             <div className="flex justify-between items-center mb-1">
                <label className="text-[10px] font-black text-slate-400 uppercase">รายละเอียด</label>
                <button type="button" onClick={handleGenerate} disabled={isGenerating} className="text-[10px] font-bold text-indigo-600 flex items-center gap-1 hover:bg-indigo-50 px-2 py-1 rounded-lg transition-colors">
                  {isGenerating ? <Loader2 className="animate-spin" size={12}/> : <Sparkles size={12}/>} AI สร้างรายละเอียด
                </button>
             </div>
             <textarea className="w-full p-3 border rounded-xl text-sm font-medium h-24" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
          </div>

          <div className="flex justify-end"><button onClick={handleSave} className="bg-indigo-600 text-white px-4 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-indigo-700"><Save size={16}/> บันทึกการแก้ไข</button></div>
    ...
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-[2.5rem] w-full max-w-md shadow-2xl animate-in zoom-in-95">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <select className="w-full p-4 border-2 rounded-xl font-bold bg-white" value={form.assignee} onChange={(e) => setForm({...form, assignee: e.target.value})}><option value="">ผู้รับผิดชอบหลัก...</option>{AUTHORIZED_USERS.map(u => <option key={u.id} value={u.name}>{u.name}</option>)}</select>
          ...
          <div className="relative">
             <button type="button" onClick={handleGenerate} disabled={isGenerating} className="absolute right-2 top-2 text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg flex items-center gap-1 hover:bg-indigo-100 transition-colors z-10">
                {isGenerating ? <Loader2 className="animate-spin" size={12}/> : <Sparkles size={12}/>} AI Create
             </button>
             <textarea className="w-full p-4 border-2 rounded-xl font-bold outline-none pt-8" placeholder="รายละเอียด..." rows="3" value={form.description} onChange={(e) => setForm({...form, description: e.target.value})} />
          </div>
      ...
      <form onSubmit={submit} className="space-y-6">
        ...
        <div className="relative">
          <div className="flex justify-between items-center mb-1">
             ...
             <button type="button" onClick={handleRefine} disabled={isRefining} className="text-[10px] font-bold text-blue-600 flex items-center gap-1 hover:bg-blue-50 px-2 py-1 rounded-lg transition-colors">
                {isRefining ? <Loader2 className="animate-spin" size={12}/> : <Sparkles size={12}/>} AI เรียบเรียง
             </button>
          </div>
          <textarea required rows="4" className="w-full p-4 border-2 border-slate-100 rounded-xl font-bold outline-none focus:border-blue-600" placeholder="ระบุรายละเอียด..." value={form.description} onChange={(e) => setForm({...form, description: e.target.value})} />
        </div>
